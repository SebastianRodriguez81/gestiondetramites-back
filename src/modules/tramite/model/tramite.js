...
function crearTramite(daoTramite) {
    return {
        ...
        async obtenerDatos(id) {
            ...
            this.revisionDate = datos.revisiondate ? moment(datos.revisiondate).format("YYYY-MM-DD HH:MM") : datos.revisiondate
            this.withdrawalDate = datos.withdrawaldate ?  moment(datos.withdrawaldate).format("YYYY-MM-DD HH:MM") : datos.withdrawaldate
        ...
        async buscarTodos(estadosIdx, tiposTramiteIdx, fechaCreacionDesde, fechaCreacionHasta, usuariosId, usuariosAsigId, id, opEstadosIdx1, opEstadosIdx2, opEstadosIdx3, idUsermunicipalRole) {
            ...
            dbResult.forEach(datos => {                
                ...
                tramiteRow.revisionDate = datos.revisiondate ? moment(datos.revisiondate).format("YYYY-MM-DD HH:MM") : datos.revisiondate
                tramiteRow.withdrawalDate = datos.withdrawaldate ? moment(datos.withdrawaldate).format("YYYY-MM-DD HH:MM") : datos.withdrawaldate
        ...
        rechazarTramite(reasonRejection) {
            if (this.idState == 5) { throw new ValidationError("El estado del tramite no permite esta accion.") }
            this.completedDate = getValidDate()
            this.rejected = true
